...
const templates: WhatsAppTemplate[] = [
  ...
  },
  {
    ...
    allowedRoles: ["admin", "coordinator"],
    getMessage: (t) =>
      `Hello ${t.customer_name},\n\nUpdate on your service request (Ticket No: ${t.ticket_number}):\n\nYour product has been sent to the vendor service center for repair.\n\nWe will update you once the repair is complete.\n\n— MAXDAX Service`,
  },
  {
    id: "completed",
    ...
    <>
      <div className="space-y-2">
        <p className="text-xs font-medium text-muted-foreground flex items-center gap-1.5 mb-2">
          <MessageSquare className="h-3.5 w-3.5" /> WhatsApp Communication
        </p>
        ...
        <div className="flex gap-2 mb-2">
          <Button
            variant="outline"
            size="sm"
            className="flex-1 text-xs h-9 gap-1.5 hover:bg-emerald-500/10 hover:border-emerald-500/30 hover:text-emerald-700 dark:hover:text-emerald-400"
            onClick={() => openWhatsAppChat(`Hello ${ticket.customer_name}, this is regarding your service ticket ${ticket.ticket_number}.`)}
          >
        ...
        <div className="grid grid-cols-2 gap-2">
          {visibleTemplates.map((template) => (
            <Button
              ...
              variant="outline"
              size="sm"
              className="text-xs h-8 justify-start gap-1.5 hover:bg-emerald-500/10 hover:border-emerald-500/30 hover:text-emerald-700 dark:hover:text-emerald-400 transition-colors"
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              To: {ticket.customer_phone} ({ticket.customer_name})
            </DialogDescription>
          </DialogHeader>
